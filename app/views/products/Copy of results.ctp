...
?>
	...
	<div class="hr-row" style="width:980px; overflow-x:auto; overflow-y:hidden;" >
		...
		<?php 
			...
			<tr>
				...
				if(strstr($final_result[$i]['who_productName'], $search_string)) {
					$replace = '<span style="color:red">'.$search_string.'</span>';
					$prodname = str_replace($search_string,$replace,$final_result[$i]['who_productName']);
				}
				print (trim($final_result[$i]['who_productName']) != "")?$prodname:'--'; ?></td>
	...
	<div class="buttonrow"><table width="100%" border="0" cellspacing="5" cellpadding="0">
		<tr>
			...
			<td width="59%" align="left"><input name="cancel" type="button" value="Cancel" onclick="javascript:redirectUrl('<?php echo SITE_URL?>/');" />&nbsp;<input name="" type="button" value="New Search" onclick="javascript:redirectUrl('<?php echo SITE_URL?>/products/search');" /></td>
		</tr>
		</table>
	</div>
</div>