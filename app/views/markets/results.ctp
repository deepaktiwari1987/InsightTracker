...
<?php 
			...
			if(isset($final_result) && count($final_result)>0) {
				$exportlink = SITE_URL."/products/exportToExcel/market";
...
?>
	<div class="hr-row">
		...
		<tr>
			<td width="50%" ><h2>Market Insight Records</h2></td>
	...
	<div class="hr-row"  style="width:980px; overflow-x:auto; overflow-y:hidden;" >
		...
		<?php 
			...
			<tr>
				<td width="2%"><a href="<?php print SITE_URL . '/markets/records/' . $final_result[$i]['id'] ; ?>"><?php print $final_result[$i]['id']; ?></a></td>
				...
				<td width="5%"><?php print $custom->getCurrentStatus($final_result[$i]['insight_status']); ?></td>
				...
				<td width="8%"><?php print (trim($final_result[$i]['whoMarket']) != "")?$final_result[$i]['whoMarket']:'--'; ?></td>
				<td width="10%"><?php print (trim($final_result[$i]['insight_summary']) != "")?substr($final_result[$i]['insight_summary'],0,50).'...':'--'; ?></td>
				<td width="8%"><?php print (trim($final_result[$i]['relates_product_family_id']) != "")?$final_result[$i]['relates_product_family_id']:'--'; ?></td>
				...
				<td width="7%">
								...
								if($final_result[$i]['attachment_real_name']!="")
									print '<a id="attachment_namelink" href="#" onclick="javascript:open_attachment(\''.SITE_URL.'/files/market/'.$final_result[$i]['attachment_name'].'\'); return false;">'.$final_result[$i]['attachment_real_name'].'</a>'; 
								else
									print "No attachment";	
								?>
				</td>
	...
	<div class="buttonrow"><table width="100%" border="0" cellspacing="5" cellpadding="0">
		<tr>
			...
			<td width="59%" align="left"><input name="cancel" type="button" value="Cancel" onclick="javascript:redirectUrl('<?php echo SITE_URL?>/customers/home');" />&nbsp;<input name="" type="button" value="New Search" onclick="javascript:redirectUrl('<?php echo SITE_URL?>/markets/search');" /></td>
		</tr>
		</table>
	</div>
</div>