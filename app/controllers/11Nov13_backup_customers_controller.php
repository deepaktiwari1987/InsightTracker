    ...
    function mhome() {
        ...
        if ($_POST && $_POST['HdnRedirect'] == "Y") {
            ...
            if (count($res) > 0 && $res['Pilotgroup']['name'] == $UserName) {
                ...
                $errArr = 'User ' . $UserName . ' do not have access to the site.';
       ...
       if (!$this->_isMobile()) {
        ...
        if (!$this->Session->check('current_user_id')) {
            ...
            if (isset($userArray) && !empty($userArray['uid']) && !empty($userArray['uname'])) {
                ...
                $this->Session->write('current_user_role', $userArray['role']);
                $this->Session->write('current_user_emailaddress', $userArray['emailaddress']);
            }
        }
        /** end * */
        /**
...
//End of results action.
    ...
    function autologin() {
        ...
        # Read LDAP settings from INI file.
        /* 		if(isset($_SERVER['LN_APP_INIFILE'])){
          $my_ini_file = $_SERVER['LN_APP_INIFILE'];
          $derived_config = parse_ini_file($my_ini_file, TRUE);

          define('LDAP_SERVER', $derived_config['ldap_info']['server']);
          define('LDAP_ACCOUNT', $derived_config['ldap_info']['account']);
          define('LDAP_PASSWORD', $derived_config['ldap_info']['password']);
          define('LDAP_PATH', $derived_config['ldap_info']['path']);
         ...
         */ /*
          #ldap settings
          $settings = array(
          'server' => '10.63.80.11',#LDAP_SERVER,
          'account' => 'Svc-leglngwoklnxp001',#LDAP_ACCOUNT,
          'password' => 'Friday301500',#LDAP_PASSWORD,
          'path' => 'OU=EMEA,OU=Users,OU=User Accounts,DC=legal,DC=regn,DC=net'#LDAP_PATH
          );
         */
        ...
        /*     if(!$ds = ldap_connect($settings['server']))
          ...
          $rec_exists = $this->Pilotgroup->find('count', array('conditions' => array( 'Pilotgroup.name like' => "%".$username."%")));
         ...
         */
        $username = ($_SERVER['REMOTE_USER'] != "") ? $_SERVER['REMOTE_USER'] : $_SERVER['AUTH_USER'];
        if($username=='WebDev'||$username==''){
             $username = 'Mathurh';
        }
       
        $res = $this->Pilotgroup->findByName($username);
        $array['uid'] = $res['Pilotgroup']['id'];
        $array['uname'] = $username; //$ad_info[0]["givenname"][0];;
        $array['role'] = $res['Pilotgroup']['role']; // User Role
        //$array['emailaddress'] = $res['Pilotgroup']['emailaddress']; // User email.
        $array['emailaddress'] = "sugandhik@damcogroup.com"; // User email.
        return $array;
    }
